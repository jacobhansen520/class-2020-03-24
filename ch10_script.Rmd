...
train %>% 
  ...
  ggplot(aes(x = treatment, y = avg_inc)) +
    geom_col()
```
...
virtual_train_sample <- train %>% 
  rep_sample_n(size = 115, replace = TRUE, reps = 2) %>% 
  group_by(replicate) %>% 
  summarize(avg_inc = mean(income))

virtual_train_sample %>%
  pull(avg_inc) %>% 
...
# As number of reps increased, confidence interval got wider
```
...
# If we survey another 115 commuters, we are 50% sure that the average of *that*
# sample would fall within this range
```
...
# Frequentist: Of the 1000 times we ran this experiment, we are 95% confident
# that any individual instance falls within this range.

# Bayesian: We are 95% confident that the true mean of the Boston population
